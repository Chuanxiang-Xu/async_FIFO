...
|---|---|
| `rtl/async_fifo.v` | Stable, minimal equal-width user entry point |
| `rtl/core/` | Storage, pointers, Gray-code synchronization, and local status generation |
| `rtl/wrappers/` | Width conversion and packet-stream protocol behavior |
| `rtl/util/` | Optional integration helpers that are not part of FIFO data movement |
...
status contracts and [CDC Constraints](cdc_constraints.md) for physical timing
requirements.
