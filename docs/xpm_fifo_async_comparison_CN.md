...
| 读模式 | 通过 `READ_MODE` 支持 standard 和 FWFT | standard 同步读加一个等宽 FWFT wrapper；没有 XPM 兼容 `READ_MODE` 参数 |
...
| `READ_MODE` | 不支持 XPM 兼容参数 | `async_fifo` 保持 standard 同步读；`async_fifo_fwft` 提供独立等宽 FWFT wrapper。 |
...
主入口 `async_fifo` 实现 standard 同步读行为：

- `rd_en && !empty` 接受读请求；
- RAM 在读时钟沿更新 `rd_data`；
- `rd_valid` 标记应该采样 `rd_data` 的周期。

具体波形见[逐步教程](tutorial_CN.md#7-一张真实-waveform)。独立
`async_fifo_fwft` wrapper 实现了
[FWFT / Fallthrough 设计说明](fwft_design_CN.md) 中定义的教学项目契约；它不是对
XPM `READ_MODE="fwft"` 每个细节的复刻。
